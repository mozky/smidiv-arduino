...
SoftwareSerial mySerial(10,11);   //software mySerial instance for GPS/GSM module
...
COBD obd;
#define DEBUG true 
String state, timegps, latitude, longitude, internetState, RPM, VELOCIDAD, CARGAMOTOR, TEMPERATURA, BATERIA, PROBLEMA;
String nombre [10] = {"DISTANCIA","GASOLINA","RPM","VELOCIDAD","CARGA DE MOTOR","TROOTTLE","TEMPERATURA ANTICONGELANTE"};

void setup() {
  mySerial.begin(9600);
  
  byte version = obd.begin();   //iniciar el puerto OBD2
  Serial.begin(9600);           //open mySerial port
  delay(50);
  
  /*We can use old sendData function to print our commands*/
  mySerial.println("Enviando info");
  sendData("AT+CGNSPWR=1",1000,DEBUG);       //Initialize GPS device
  delay(50);
  mySerial.println("Enviando info2");
  sendData("AT+CGNSSEQ=RMC",1000,DEBUG);
  delay(150);
   // initialize OBD-II adapter
  obd.init();
  /*do {
    mymySerial.println("Connecting...");
  } while (!obd.init());
  mymySerial.println("OBD connected!");
*/
...
void getGPS()
{
  sendTabData("AT+CGNSINF",5000,DEBUG);    //send demand of gps localization & GPRS connection (no funciona todavía)
   ...
   for(int i = 0; i < (sizeof(latlongtab)/sizeof(int)); i++){
    ...
    mySerial.println("GPS & Internet initializing");
   }
  }
...
void connectNetwork() {
  ...
  mySerial.print("Internet response: ");  
  mySerial.print(response);
